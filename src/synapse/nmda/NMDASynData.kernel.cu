...
__global__ void update_denser_nmda_hit(Connection *connection, NMDAData *data, real *buffer, const uinteger_t *firedTable, const uinteger_t *firedTableSizes, size_t firedTableCap, size_t num, size_t start_id, int time)
{
	int delayLength = connection->maxDelay - connection->minDelay + 1;
	for (int delta_t = 0; delta_t<delayLength; delta_t++) {
		int block_idx = blockIdx.x;
...
__global__ void update_nmda_2l(Connection *connection, NMDAData *data, real *buffer, const uinteger_t *firedTable, const uinteger_t *firedTableSizes, size_t firedTableCap, size_t num, size_t start_id, int time)
{
	int delayLength = connection->maxDelay - connection->minDelay + 1;
...
void cudaUpdateNMDA(Connection * connection, void *data, real *buffer, uinteger_t *firedTable, uinteger_t *firedTableSizes, size_t firedTableCap, size_t num, size_t start_id, int time, BlockSize *pSize)
{
	...
	//update_nmda_hit<<<pSize->gridSize, pSize->blockSize>>>((NMDAData*)data, num, start_id);
	//reset_active_synapse<<<1, 1>>>();
	// update_denser_nmda_hit<<<pSize->gridSize, pSize->blockSize>>>((Connection *)connection,  (NMDAData *)data, buffer, firedTable, firedTableSizes, firedTableCap, num, start_id, time);
	...
	// update_nmda_2l<<<grid, block>>>((Connection *)connection,  (NMDAData *)data, buffer, firedTable, firedTableSizes, firedTableCap, num, start_id, time);
}
