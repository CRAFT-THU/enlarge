...
__global__ void update_dense_nmda_hit(Connection *connection, NMDAData *data, real *buffer, const uinteger_t *firedTable, const uinteger_t *firedTableSizes, size_t firedTableCap, size_t num, size_t start_id, int time)
{
    // * 整体先衰减
    ...
    for (int idx = 0; idx < data->num; idx += blockDim.x * gridDim.x) {
        ...
        if (sid < data->num) {
            real half_x = data->pX[sid] * 0.5;
            data->pS[sid] = (data->pC_decay[sid] - half_x) * data->pS[sid] + half_x;
            data->pX[sid] *= data->pC_rise[sid];
        }
    }
		...
		if (firedSize > 0) {
			...
			for (uinteger_t idx = 0; idx < fired_size_block; idx++) {
				...
				for (uinteger_t j=threadIdx.x; j<synapseNum; j += blockDim.x) {
					...
					data->pX[connection->pSidMap[sid]] += 1;
                    real inc = data->pG[connection->pSidMap[sid]] * data->pS[connection->pSidMap[sid]];
...
__global__ void update_denser_exp_hit(Connection *connection, NMDAData *data, real *buffer, const uinteger_t *firedTable, const uinteger_t *firedTableSizes, size_t firedTableCap, size_t num, size_t start_id, int time)
{
...
__global__ void update_exp_2l(Connection *connection, NMDAData *data, real *buffer, const uinteger_t *firedTable, const uinteger_t *firedTableSizes, size_t firedTableCap, size_t num, size_t start_id, int time)
{
...
void cudaUpdateNMDA(Connection * connection, void *data, real *buffer, uinteger_t *firedTable, uinteger_t *firedTableSizes, size_t firedTableCap, size_t num, size_t start_id, int time, BlockSize *pSize)
{
	// std::cout << pSize->gridSize << " " << pSize->blockSize << std::endl;
	// update_exp_hit<<<pSize->gridSize, pSize->blockSize>>>((NMDAData*)data, num, start_id);
	// reset_active_synapse<<<1, 1>>>();
	// update_denser_exp_hit<<<pSize->gridSize, pSize->blockSize>>>((Connection *)connection,  (NMDAData *)data, buffer, firedTable, firedTableSizes, firedTableCap, num, start_id, time);
	// std::cout << "NUMBER in updateNMDA: " << num << std::endl;


	update_dense_nmda_hit<<<pSize->gridSize, pSize->blockSize>>>((Connection *)connection,  (NMDAData *)data, buffer, firedTable, firedTableSizes, firedTableCap, num, start_id, time);
	...
	// update_exp_2l<<<grid, block>>>((Connection *)connection,  (NMDAData *)data, buffer, firedTable, firedTableSizes, firedTableCap, num, start_id, time);
}
