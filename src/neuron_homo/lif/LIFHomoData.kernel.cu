...
__global__ void update_all_lif_homo_neuron(Connection *connection, LIFHomoData *data, real *buffer, uinteger_t *firedTable, uinteger_t *firedTableSizes, size_t firedTableCap, size_t num, size_t offset, int time)
	...
	for (size_t idx = 0; idx < num; idx +=blockDim.x*gridDim.x) {
		...
		if (nid < num) {
			...
			if (actived) {
				...
				fired = data->pV_m[nid] >= data->cV_thresh;
...
void cudaUpdateLIFHomo(Connection *conn, void *data, real *buffer, uinteger_t *firedTable, uinteger_t *firedTableSizes, size_t firedTableCap, size_t num, size_t offset, int time, BlockSize *pSize)
{
	...
	update_all_lif_homo_neuron<<<pSize->gridSize, pSize->blockSize>>>(conn, (LIFHomoData*)data, buffer, firedTable, firedTableSizes, firedTableCap, num, offset, time);
