...
int copyNMDA(void *p_src, size_t s_off, void *p_dst, size_t d_off) 
{
	NMDAData *src = static_cast<NMDAData *>(p_src);
	NMDAData *dst = static_cast<NMDAData *>(p_dst);
...
{
	char filename[512];
	sprintf(filename, "rate_%s.%s.log", name, "NMDA");
	FILE *f = fopen_c(filename, "w+");
	NMDAData *d = static_cast<NMDAData*>(data);
