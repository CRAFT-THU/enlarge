...
LIFExpNeuron::LIFExpNeuron(real v_init, real v_rest, real v_reset, real v_th,
                           real r, real tau_m, real tau_ref, real E_syn, real dt,
                           size_t num)
    : Neuron(LIFExp, num) {
    real cm = dt / tau_m;
...
LIFExpNeuron::LIFExpNeuron(const LIFExpNeuron &n, size_t num)
    : Neuron(LIFExp, 0) {
    append(dynamic_cast<const Neuron *>(&n), num);
}

LIFExpNeuron::~LIFExpNeuron() {
    _num = 0;
    _refract_time.clear();
    _refract_step.clear();
    _v.clear();
...
int LIFExpNeuron::append(const Neuron *neuron, size_t num) {
    const LIFExpNeuron *n = dynamic_cast<const LIFExpNeuron *>(neuron);
    int ret = 0;
    if ((num > 0) && (num != n->size())) {
        ret = num;
        _refract_step.insert(_refract_step.end(), num, 0);
...
void *LIFExpNeuron::packup() {
    LIFExpData *p = static_cast<LIFExpData *>(mallocLIFExp());
    assert(p != NULL);
    p->num = _num;
...
int LIFExpNeuron::packup(void *data, size_t dst, size_t src) {
