...
{
	void *ret = NULL;
	checkCudaErrors(cudaMalloc((void**)&(ret), sizeof(LIFNmdaData)*1));
	checkCudaErrors(cudaMemset(ret, 0, sizeof(LIFNmdaData)*1));
	return ret;
}

void *cudaAllocLIFNmda(void *pCPU, size_t num)
{
	void *ret = cudaMallocLIFNmda();
	void *tmp = cudaAllocLIFNmdaPara(pCPU, num);
	checkCudaErrors(cudaMemcpy(ret, tmp, sizeof(LIFNmdaData)*1, cudaMemcpyHostToDevice));
	free(tmp);
	tmp = NULL;
	return ret;
}
...
void *cudaAllocLIFNmdaPara(void *pCPU, size_t num)
{
	...
	LIFNmdaData *ret = (LIFNmdaData*)malloc(sizeof(LIFNmdaData)*1);
	memset(ret, 0, sizeof(LIFNmdaData)*1);
    ...
    checkCudaErrors(cudaMalloc((void**)&(ret->pM_c), sizeof(real)*num));
    checkCudaErrors(cudaMemset(ret->pM_c, 0, sizeof(real)*num));
    checkCudaErrors(cudaMemcpy(ret->pM_c, p->pM_c, sizeof(real)*num, cudaMemcpyHostToDevice));
...
{
	LIFNmdaData *pTmp = (LIFNmdaData*)malloc(sizeof(LIFNmdaData)*1);
	memset(pTmp, 0, sizeof(LIFNmdaData)*1);
	checkCudaErrors(cudaMemcpy(pTmp, pGPU, sizeof(LIFNmdaData)*1, cudaMemcpyDeviceToHost));

	cudaLIFNmdaParaFromGPU(pCPU, pTmp, num);
	return 0;
}

int cudaLIFNmdaParaToGPU(void *pCPU, void *pGPU, size_t num)
{
	LIFNmdaData *pC = (LIFNmdaData*)pCPU;
	LIFNmdaData *pG = (LIFNmdaData*)pGPU;
    ...
    checkCudaErrors(cudaMemcpy(pG->pM_c, pC->pM_c, sizeof(real)*num, cudaMemcpyHostToDevice));
...
int cudaLIFNmdaParaFromGPU(void *pCPU, void *pGPU, size_t num)
{
    ...
    checkCudaErrors(cudaMemcpy(pC->pM_c, pG->pM_c, sizeof(real)*num, cudaMemcpyDeviceToHost));
...
{
	LIFNmdaData *tmp = (LIFNmdaData*)malloc(sizeof(LIFNmdaData)*1);
	memset(tmp, 0, sizeof(LIFNmdaData)*1);
	checkCudaErrors(cudaMemcpy(tmp, pGPU, sizeof(LIFNmdaData)*1, cudaMemcpyDeviceToHost));
	cudaFreeLIFNmdaPara(tmp);
	tmp = free_c(tmp);
	cudaFree(pGPU);
	pGPU = NULL;
	return 0;
...
{
	LIFNmdaData *p = (LIFNmdaData*)pGPU;
...
{
	LIFNmdaData *c = static_cast<LIFNmdaData *>(cpu);
	LIFNmdaData *g = static_cast<LIFNmdaData *>(gpu);

	LIFNmdaData *t = FROMGPU(g, 1);
	COPYFROMGPU(c->_fire_count, t->_fire_count, c->num);
	return logRateLIFNmda(cpu, name);
}

real * cudaGetVLIFNmda(void *data) {
	LIFNmdaData *c_g_lif = FROMGPU(static_cast<LIFNmdaData *>(data), 1);
	return c_g_lif->pV;
}
